...
remetentes_excluidos = [
    "no-reply@onedrive.com", "msa@communication.microsoft.com", "ana.rebeca@bagaggio.com.br", "andrew.germano@bagaggio.com.br", "anny.kelly@bagaggio.com.br", 
    ...
    "vanessa.rebello@bagaggio.com.br", "centraldenotasbagaggio@outlook.com", "centraldenotas@bagaggio.com.br", "paulo.mattos@bagaggio.com.br", "analiseretencaofiscal@bagaggio.com.br",
    "One Drive", "ANA REBECA", "Andrew Germano", "ANNY KELLY", "Crystiano Bento", "Douglas França", "GUILHERME  ALMEIDA", "JULIANA ALMEIDA", "Julio Correia",
    "Matheus Silva", "Monique Barbosa", "PEDRO RUFINO", "Thamirys Abreu", "Vanessa Rebello", "Central Notas", "Central De Notas", "Paulo Mattos", "Análise de Retenção Fiscal"
]
...
# Configuração centralizada, usa variáveis de ambiente
db_config = {
...
# String de conexão baseada no dicionário
conn_string = (
...
def inserir_no_banco(df, conn_string):
    ...
    for index, row in df.iterrows():
        ...
        cursor.execute('''
            INSERT INTO ProdF_MapeamentoNotas (Assunto, Data, Remetente, Anexos, NotasFiscais, 
    ...
    try:
        ...
        delete_query = '''
            WITH CTE AS (
                ...
                FROM ProdF_MapeamentoNotas
            )
            DELETE FROM CTE WHERE row_num > 1;
        '''
        print("Removendo linhas duplicadas da tabela ProdF_MapeamentoNotas")
        cursor.execute(delete_query)
        conn.commit()
        cursor.close()
        conn.close()

        print("Linhas duplicadas removidas da tabela ProdF_MapeamentoNotas com sucesso.")
    except Exception as e:
...
def main():
    ...
    print("5. Extrair dados para Excel")

    choice = '4'
    #choice = input("Digite 1, 2, 3, 4 ou 5: ").strip()

    if choice == '5':
        # Submenu para a opção de extrair para Excel
        print("\nVocê escolheu: Extrair dados para Excel")
        print("\nEscolha a forma de filtrar os dados:")
        print("1. Usar a data D-1")
        print("2. Inserir um intervalo de datas")
        print("3. Inserir um dia específico que deseje consultar")

        excel_choice = input("Digite 1, 2 ou 3: ").strip()

        if excel_choice == '1':
            date_range = [(datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')]  # Data D-1
        elif excel_choice == '2':
    ...
    email_label_1 = os.getenv("EMAIL_LABEL_1")
    email_user_2 = os.getenv("EMAIL_USER_2")
    email_pass_2 = os.getenv("EMAIL_PASS_2")
    email_label_2 = os.getenv("EMAIL_LABEL_2")
    ...
    if choice == '5':  # Exportar para Excel
        file_name = os.path.join(os.getcwd(), "PRIVATE-Email-Automation","extracao_notas_fiscais.xlsx")
        try:
            df.to_excel(file_name, index=False)
            print(f"Arquivo Excel gerado com sucesso: {file_name}")
        except Exception as e:
